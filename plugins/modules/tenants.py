...
options:
  ...
  mtls_identifier:
    description: "The mTLS identifier of the tenant Requires VMS 5.5 or later."
    type: str
  ...
  force_remove:
    description: |
      Forces deletion of the tenant, even if files exist in the tenant directory. (only used when state=absent)
    type: bool
  state:
...
ARGUMENT_SPEC: Dict[str, Any] = {
    ...
    "force_remove": {"type": "bool", "default": None},
...
class TenantResource(BaseResource):
    ...
    lookup_field = "name"
    async_delete = True
    create_only_fields = {"encryption_crn", "encryption_group", "name"}  # Fields only valid during creation
    update_only_fields = {"data_engine_role_enabled", "data_engine_s3_policy_enabled"}  # Fields only valid during updates
    delete_query_params = {"force_remove"}  # Sent as URL query params on DELETE
