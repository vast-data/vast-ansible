...
DOCUMENTATION = r"""
---
module: iamroles_info
short_description: List VAST IAMRole resources
description:
  - List VAST IAMRole resources.
...
options:
  description:
    description: "Filter by description (optional filter)"
    type: str
  ...
  max_session_duration:
    description: |
      Maximum session duration for temporary access keys (seconds). Default - 129600 (36 hours). (optional filter)
    type: int
...
EXAMPLES = r"""
- name: List all iamroles
  vastdata.vms.iamroles_info:
    vms:
      host: vast-vms.example.com
      token: "{{ vast_token }}"
  register: iamroles_result

- name: List iamroles filtered by name
  vastdata.vms.iamroles_info:
    vms:
      host: vast-vms.example.com
      token: "{{ vast_token }}"
    name: example-value
  register: filtered_iamroles
"""
...
changed:
  ...
  returned: always
iamroles:
  description: List of matching iamroles resources.
  type: list
  elements: dict
  returned: always
  sample: []
...
class IamroleInfoResource(BaseInfoResource):
    """Read-only list manager for IAMRole."""
...
def main() -> None:
    ...
    try:
        resource = IamroleInfoResource(module)
        resource.run()
