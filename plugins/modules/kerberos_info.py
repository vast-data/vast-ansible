...
DOCUMENTATION = r"""
---
module: kerberos_info
short_description: List VAST Kerberos resources
description:
  - List VAST Kerberos resources.
...
- name: List kerberos filtered by realm
  vastdata.vms.kerberos_info:
    vms:
      host: vast-vms.example.com
      token: "{{ vast_token }}"
    realm: example-value
  register: filtered_kerberos
"""
...
changed:
  ...
  returned: always
kerberos:
  description: List of matching kerberos resources.
  type: list
  elements: dict
  returned: always
  sample: []
...
class KerberosInfoResource(BaseInfoResource):
    """Read-only list manager for Kerberos."""

    resource_name = "kerberos"
    return_key = "kerberos"
    filter_fields = {"enabled", "guid", "id", "realm"}
...
def main() -> None:
    ...
    try:
        resource = KerberosInfoResource(module)
        resource.run()
