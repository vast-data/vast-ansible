...
#
# Sub-endpoint module: supportbundlesqueue/move
...
DOCUMENTATION = r"""
---
module: supportbundlesqueue_move
short_description: Manage supportbundlesqueue move
description:
  - Manage supportbundlesqueue move.
  - Supports check_mode and diff.
version_added: "1.0.0"
options:
  supportbundlesqueue_id:
    description: "ID of the supportbundlesqueue"
    type: int
    required: true
  position:
...
EXAMPLES = r"""
- name: Manage supportbundlesqueue move
  vastdata.vms.supportbundlesqueue_move:
    vms:
      host: vast-vms.example.com
      token: "{{ vast_token }}"
    supportbundlesqueue_id: 1
"""
...
ARGUMENT_SPEC = {
    ...
    "supportbundlesqueue_id": {"required": True, "type": "int"},
...
class SupportbundlesqueueMove(SubEndpointResource):
    parent_resource = "supportbundlesqueue"
    sub_path = "move"
    path_has_id = True
    supported_operations = frozenset({CrudCapability.UPDATE})
    parent_id_param = "supportbundlesqueue_id"
...
def main() -> None:
    ...
    try:
        resource = SupportbundlesqueueMove(module)
        resource.run()
